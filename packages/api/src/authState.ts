 ...
 *
 * An in-memory-only limiter resets on every restart, and Render's free tier restarts
 * whenever the instance spins down or a deploy lands — so the window a determined attacker
 * had to wait out would be minutes, not the quarter hour it claims. Firestore is already a
 * dependency and login volume is tiny, so the cost is negligible. Locally, with no Firestore
 * configured, this reads/writes the in-memory dev store instead (see firebaseAdmin.ts) —
 * still cross-request, just not cross-restart.
 *
 * Every read fails open. If the store is unreachable the owner must still be able to log in
 * and fix things; a lockout that triggers precisely when the database is down is worse than
 * the brute-force window it closes. The in-memory burst limiter in routes/auth.ts still
 * applies, so failing open is not the same as no limit.
 */
 ...
 * import time and throws outright when the service-account variables are malformed, which
 * would make every module that touches auth impossible to load — including in tests.
 * Deferring it also means a misconfigured environment surfaces on the first request instead
 * of taking the whole process down at startup.
 */
...
/** Test seam: pass a stand-in, or null to fall back to firebaseAdmin's db. */
...
export async function currentEpoch(): Promise<number> {
  ...
  try {
    ...
    // Fail open to the last known value, or 0. A store blip should not log the owner out of
    // a portal they may need in order to diagnose it.
 ...
 *
 * The alternative was rotating JWT_SECRET, which works but needs a dashboard visit and a
 * restart. This is the thing to reach for if a laptop goes missing.
 */
