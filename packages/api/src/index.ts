...
import { db, isFirestoreConfigured } from './firebaseAdmin';
...
const allowedOrigins = [
  // Vite's default dev port. Not actually load-bearing for `npm run dev` — the web dev
  // server proxies /api to this one (vite.config.ts), so the browser's request is
  // same-origin and never triggers a CORS check at all — but worth being correct here in
  // case anything ever talks to this API directly from a browser at that origin.
  'http://localhost:5173',
 ...
 *
 * This exists because GET /api/content swallows store errors and serves the bundled
 * fallback with a 200. That is right for visitors, but it means the site can look perfectly
 * healthy while every admin edit has silently stopped appearing, with nothing to alert on.
 * This endpoint goes red in that case.
 */
...
app.get('/health/deps', async (_req, res) => {
  ...
  try {
    ...
    // reachability. Points at a doc the app actually reads, not one kept alive only for
    // this check.
    await Promise.race([
...
app.listen(PORT, () => {
  ...
  // works on a fresh clone — these are the things that are silently inert (or, for
  // Firestore, running on a placeholder) until set.
  ...
  if (!isFirestoreConfigured) {
    console.warn(
      '[content] FIREBASE_* is unset — using an in-memory content store as a local ' +
        'placeholder. Edits work, but reset every time this process restarts. Set FIREBASE_* ' +
        'to a real Firestore project before deploying.',
    );
  }
  if (!process.env.CLOUDINARY_URL) {
    console.warn('[content] CLOUDINARY_URL is unset — image/CV upload will answer 503.');
  }
});
