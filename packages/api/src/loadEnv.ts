...
// `node packages/api/dist/index.js` from the repo root — would find no .env. Both src/ and
// dist/ sit one level under packages/api, so '../.env' is correct whether running from
// source or compiled.
//
// This lives in its own module because it has to run before anything that reads
// process.env at import time (firebaseAdmin does). Imports are evaluated before statements,
// so calling dotenv.config() inside index.ts would run too late.
//
